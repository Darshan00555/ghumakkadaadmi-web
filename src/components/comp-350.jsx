...
export default function Component() {
  return (
    <div className="space-y-4">
      ...
      <Accordion className="-space-y-px w-full" collapsible defaultValue="3" type="single">
        {items.map((item) => (
          <AccordionItem
            ...
            value={item.id}>
            <AccordionTrigger
              className="[&>svg]:-order-1 justify-start gap-3 rounded-md py-2 text-[15px] leading-6 outline-none hover:no-underline focus-visible:ring-0">
              {item.title}
            </AccordionTrigger>
